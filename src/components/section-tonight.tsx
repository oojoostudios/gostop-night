      ...
      <div className="lg:ml-72">
        <div className="mx-auto max-w-5xl px-6 sm:px-8 lg:px-16">
          ...
          <FadeInOnView
            as="p"
            delay={0.12}
            className="mb-10 max-w-2xl text-body leading-relaxed text-ink-soft"
          >
            {t(
  ...
  return (
    <div className="space-y-10">
      <div className="club-card flex flex-col items-center gap-4 p-8 text-center">
        ...
        <p className="max-w-[40ch] text-body text-ink-soft">
          {t(
        ...
        <button
          type="button"
          className="club-btn club-btn--primary mt-2 text-body"
          onClick={onStart}
        >
      ...
      {past.length > 0 && (
        <div>
          <h3 className="mb-4 font-display text-sub">{t('Past events', '지난 게임')}</h3>
          <ul className="grid gap-3 sm:grid-cols-2" data-testid="past-events">
            {past.map((ev) => (
              <li key={ev.id}>
                ...
                >
                  <div className="truncate font-display text-sub">{ev.name}</div>
                  <div className="mt-0.5 text-label tabular-nums text-ink-soft">{ev.date}</div>
                  <div className="mt-3 text-body tabular-nums">
                    {standings(ev)
