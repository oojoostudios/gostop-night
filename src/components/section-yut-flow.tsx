...
const NORMAL_STEPS: Step[] = [
  {
    ...
    titleKo: '둘러앉아요',
    desc: 'Each player gets 4 pieces stacked at start (출발). P1 (red) throws first.',
    descKo: '각 플레이어가 4개의 말을 출발점에 두고 앉아요. P1(빨강)이 먼저 던져요.',
  ...
  },
  {
    ...
    title: 'P1 throws — gae',
    titleKo: 'P1이 던져서 — 개',
    desc: 'Two sticks land flat-up: gae (개). Move 2 spaces forward.',
    descKo: "막대 두 개가 평평한 면이 위로 — '개'. 2칸 전진할 수 있어요.",
  ...
  },
  {
    ...
    titleKo: '말 하나를 골라요',
    desc: 'P1 picks one of their 4 pieces to move. (Strategy comes later — for now, just pick any.)',
    descKo: 'P1이 4개의 말 중 하나를 골라요. (전략은 나중 — 일단 아무거나 골라봐요.)',
    state: { pieceHistory: [], whoseTurn: 'p1', lastThrow: T_GAE },
  },
  {
    id: 'move',
    ...
    desc: 'The chosen piece walks two stations along the outer ring. Always counterclockwise.',
    descKo: '고른 말이 외곽길을 따라 2칸 움직여요. 항상 시계 반대방향.',
    state: {
  ...
  },
  {
    ...
    titleKo: 'P2 차례로 넘어가요',
    desc: "No bonus on gae, so P1's turn ends. P2 (blue) throws next.",
    descKo: '개에는 보너스가 없어서 P1의 차례가 끝나요. 이제 P2(파랑)가 던져요.',
    state: {
      pieceHistory: ['o-r4', 'o-r3'],
      whoseTurn: 'p2',
      lastThrow: T_GAE,
...
const BONUS_STEPS: Step[] = [
  ...
  },
  {
    ...
    title: 'P1 throws — yut!',
    titleKo: 'P1이 던져서 — 윷!',
    desc: "All four sticks flat-up: yut (윷). Move 4 spaces AND throw again — that's the bonus.",
    descKo: "네 막대 모두 평평한 면이 위 — '윷'. 4칸 + 한 번 더 던지기 보너스!",
    state: {
      pieceHistory: [],
      whoseTurn: 'p1',
      lastThrow: T_YUT,
  ...
  },
  {
    ...
    title: 'Move 4 spaces',
    titleKo: '4칸 이동',
    desc: 'P1 moves a piece 4 stations. Now they get to throw again before passing.',
    descKo: 'P1의 말이 4칸 전진. 차례를 넘기기 전에 한 번 더 던질 수 있어요.',
    state: {
  ...
  },
  {
    ...
    titleKo: '보너스 던지기 — 도',
    desc: 'Bonus throw lands as do (도). Move 1 more space. (Could have been another yut/mo and chained again!)',
    descKo: '보너스 던지기 결과는 도. 1칸 더 이동. (또 윷/모면 또 한 번 더 가능해요.)',
    state: {
      pieceHistory: ['o-r4', 'o-r3', 'o-r2', 'o-r1'],
      whoseTurn: 'p1',
      lastThrow: T_DO,
  ...
  },
  {
    ...
    desc: 'The piece reaches the NE corner exactly. (That triggers the diagonal shortcut option — see section 02.)',
    descKo: '말이 정확히 NE 코너에 도착. (이러면 지름길 옵션이 떠요 — 섹션 02 참고.)',
    state: {
  ...
  },
  {
    ...
    title: 'Turn passes',
    titleKo: 'P2 차례로',
    desc: "Bonus throw used up — P1's turn ends. With yut+do, P1 covered 5 spaces in one turn.",
    descKo: '보너스도 다 썼으니 P1 차례 끝. 윷+도로 한 차례에 5칸을 갔네요.',
    state: {
      pieceHistory: ['o-r4', 'o-r3', 'o-r2', 'o-r1', 'ne'],
      whoseTurn: 'p2',
      lastThrow: T_DO,
...
const SCENARIOS: ReadonlyArray<Scenario> = [
  {
    ...
    label: 'Normal turn',
    labelKo: '기본 차례',
    blurb: 'Throw the sticks, move a piece that many spaces, pass to the next player.',
    blurbKo: '윷을 던지고, 그만큼 말을 옮기고, 다음 사람에게 넘겨요.',
    steps: NORMAL_STEPS,
  },
  {
    id: 'bonus',
...
export function SectionYutFlow() {
  ...
  return (
    <section id="yut-flow" className="relative py-24 border-t border-foreground/10">
      <div className="lg:ml-72">
        <div className="max-w-5xl mx-auto px-6 sm:px-8 lg:px-16">
          <div className="text-xs tabular-nums text-foreground/50 mb-4">SECTION 03</div>
...
function PlayerBadge({ player }: { player: Player }) {
  ...
  return (
    ...
    >
      <span className={`size-2 rounded-full ${isP1 ? 'bg-rose-500' : 'bg-blue-500'}`} />
      {locale === 'ko' ? (isP1 ? 'P1 차례' : 'P2 차례') : isP1 ? "P1's turn" : "P2's turn"}
    </div>
  );
}
  ...
  return (
    <div className="hidden md:flex flex-col gap-2 min-w-[180px]">
      ...
      <Stat
        ...
        value={state.bonusPending ? (locale === 'ko' ? '한 번 더' : 'Extra throw') : '—'}
        highlight={state.bonusPending}
      />
    </div>
  );
