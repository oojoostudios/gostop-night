...
export function SectionYutSticks() {
  ...
  return (
    <section id="yut-sticks" className="relative min-h-screen flex flex-col justify-center py-24">
      <div className="lg:ml-72">
        <div className="max-w-5xl mx-auto px-6 sm:px-8 lg:px-16">
          <div className="text-xs tabular-nums text-foreground/50 mb-4">SECTION 01</div>
    ...
    >
      <div className="flex gap-1.5 items-end">
        {throwKind.sticks.map((flat, i) => (
          // eslint-disable-next-line react/no-array-index-key -- 4 fixed stick slots; position IS identity
          <YutStick key={i} flatUp={flat} marked={throwKind.markedStickIndex === i} size="sm" />
        ))}
      </div>
      ...
      <div className="text-right">
        ...
        <div className="text-[10px] text-foreground/50 mt-0.5">{moveLabel}</div>
      </div>
    </div>
  );
}
