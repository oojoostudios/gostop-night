...
const STEPS: Step[] = [
  {
    ...
    titleKo: '각자 말 4개로 시작',
    desc: 'Yutnori is a race: bring all four of your pieces home to win. Pieces start stacked at 출발 (start).',
    descKo:
      '윷놀이는 경주예요. 자신의 말 4개를 모두 출발점으로 돌려보내면 승리. 처음엔 출발점에 4개가 함께 놓여있어요.',
    state: {
      pieces: [{ id: 'a', history: [], player: 'p1', stack: 4 }],
      homed: 0,
    },
  },
  {
    ...
    titleKo: '첫 말이 골인',
    desc: "After several turns, P1's first piece completes a full lap and returns to start. 1 of 4 done — three more to go.",
    descKo:
  ...
  },
  {
    ...
    titleKo: '둘째 말 — 절반',
    desc: 'P1 takes a shortcut from NE and brings the second piece home. 2 of 4. Strategy is paying off.',
    descKo: 'P1이 NE 코너에서 지름길을 타서 둘째 말도 골인. 2/4. 전략이 효과를 보고 있어요.',
    state: {
      pieces: [
  ...
  },
  {
    ...
    desc: 'Three pieces home. The last piece is on the bottom row, almost there.',
    descKo: '세 말이 골인. 마지막 말이 바닥 row에 있고, 거의 다 왔어요.',
    state: {
      pieces: [
  ...
  },
  {
    ...
    desc: 'The fourth piece crosses the finish line. P1 wins the round. 게임 끝!',
    descKo: '마지막 말도 골인. P1이 이번 라운드 승리. 끝!',
    state: {
      pieces: [{ id: 'abcd', history: ['start'], player: 'p1', completed: true, stack: 4 }],
      homed: 4,
      won: true,
    },
  },
...
export function SectionYutWinning() {
  ...
  return (
    <section id="yut-winning" className="relative py-24 border-t border-foreground/10">
      <div className="lg:ml-72">
        <div className="max-w-5xl mx-auto px-6 sm:px-8 lg:px-16">
          <div className="text-xs tabular-nums text-foreground/50 mb-4">SECTION 05</div>
...
function ProgressMeter({ homed }: { homed: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
        ...
        {[0, 1, 2, 3].map((i) => (
          <motion.div
            ...
            className={`size-3 rounded-full border-2 transition-colors ${
              i < homed ? 'bg-amber-500 border-amber-700' : 'bg-transparent border-foreground/25'
            }`}
            animate={{
      ...
      <span className="text-sm font-semibold tabular-nums">{homed} / 4</span>
    </div>
  );
}
